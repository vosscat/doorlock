                ...
                elseif paused then if not IsPauseMenuActive() then paused = false end
                    ...
                    if closestDoor.distance < closestDoor.data.maxDistance then
                        ...
                        if not closestDoor.data.locked and not canOpen then
                            if Config.ShowUnlockedText and (closestDoor.data.showNUI == nil or closestDoor.data.showNUI) then displayNUIText('Lock') else hideNUI() end
                        elseif not closestDoor.data.locked and canOpen then
                            if Config.ShowUnlockedText and (closestDoor.data.showNUI == nil or closestDoor.data.showNUI) then displayNUIText('[E] - Lock') else hideNUI() end
                        elseif closestDoor.data.locked and not canOpen and (closestDoor.data.showNUI == nil or closestDoor.data.showNUI) then displayNUIText('Unlock')
                        elseif closestDoor.data.locked and canOpen and (closestDoor.data.showNUI == nil or closestDoor.data.showNUI) then displayNUIText('[E] - Unlock') end
        ...
        if distance < 10 then
            if not door.audioLock then
                if door.audioRemote then
                    door.audioLock = {['file'] = 'button-remote.ogg', ['volume'] = 0.2}
                    door.audioUnlock = {['file'] = 'button-remote.ogg', ['volume'] = 0.2}
                else
                    door.audioLock = {['file'] = 'door-bolt-4.ogg', ['volume'] = 0.2}
                    door.audioUnlock = {['file'] = 'door-bolt-4.ogg', ['volume'] = 0.2}
                end
            end
...
local function LockpickFinish(success)
    if success then
        ...
        QBCore.Functions.Notify('失败..', 'error', 2500)
    end
end
...
RegisterKeyMapping('doorlock', 'Door Lock', 'keyboard', 'E')
